...
/**
 * Extended the BasicAuthenticationFilter to make Spring replace it in the filter chain with our
 * custom implementation. The most important part of the filter that we've implemented is the private
 * getAuthentication method. This method reads the JWT from the Authorization header, and then uses
 * Jwts to validate the token. If everything is in place, we set the user in the SecurityContext
